...
软件零点只在当前 `MotionHost` 会话内有效，不写入磁盘；主程序重启、后台进程重启或控制器重连后必须重新回零。标定出的机械行程和软件限位保存在当前用户目录：

```text
%LOCALAPPDATA%/ThermoVision/motion-settings.xml
```

行程标定会在正、负机械限位两端各保留 30 个控制器单位的安全余量。旧版配置因为没有保存独立的机械行程，会自动视为无效并要求重新标定。换控制器或调整机械结构后，也应重新标定；不要直接复制其他设备上的行程参数。
...
- FMC4030 返回的位置是控制卡内部脉冲计数，电机堵转或机械卡滞时坐标仍可能继续变化；现有接线没有编码器或驱动器报警反馈，软件无法可靠识别这种堵转。
